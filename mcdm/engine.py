...
SUPPORTED_METHODS = ("promethee", "vikor", "topsis", "waspas", "codas")
SUPPORTED_WEIGHTINGS = ("critic", "entropy")


def rank_players(player_df, criteria_config, method="promethee",
                 custom_weights=None, weighting="critic"):
    """
    ...
    Parameters:
        ...
        weighting:       str           - 'critic' (default) or 'entropy'
    ...
    # Objective baseline weights.
    if weighting == "entropy":
        objective_w = entropy_weights(matrix, types)
    else:
        objective_w = critic_weights(matrix, types)

    # Slider overrides, then renormalize.
    if custom_weights:
        weights = np.array([custom_weights.get(c, objective_w[i])
    ...
    if method == "promethee":
        ...
        result["score"] = 1 - Q  # invert so higher = better (display consistency)
        result["q_value"] = Q
    elif method == "topsis":
